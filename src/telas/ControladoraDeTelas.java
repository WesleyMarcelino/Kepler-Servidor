...
public class ControladoraDeTelas {
	...
	public static void iniciarTelaChat(Equipe equipe, Socket cliente,
			Servidor servidor) throws IOException {
	...
	public static void mostrarTelaChat() throws IOException {// pedir id
		...
		/*
		 * for (int i =0; i>=ListaTelaDeChat.getTelasDeChat().size();i++){ if
		 * (ListaTelaDeChat.getTelasDeChat().get(i).getIdTelaDechat() == id){
		 * ListaTelaDeChat.getTelasDeChat().get(i).setVisible(true); break; } }
		 */
	}
